...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
        ...
        // ==========================================
        // API：前台动态增删基金 (V1.3 新增)
        // ==========================================
        if (request.method === 'POST' && url.pathname === '/api/manage_fund') {
            const { code, name } = await request.json();
            await env.DB.prepare("INSERT OR IGNORE INTO funds (fund_code, fund_name, principal, shares) VALUES (?, ?, 0, 0)").bind(code, name).run();
            return new Response(JSON.stringify({ success: true }));
        }
        ...
        if (request.method === 'DELETE' && url.pathname.startsWith('/api/manage_fund/')) {
            ...
            // 物理移除基金总盘和关联的流水记录
            await env.DB.prepare("DELETE FROM funds WHERE fund_code = ?").bind(code).run();
            await env.DB.prepare("DELETE FROM fund_logs WHERE fund_code = ?").bind(code).run();
            return new Response(JSON.stringify({ success: true }));
        }
        ...
        if (request.method === 'GET') {
            ...
            // V1.3 移除了对基金列表的硬编码干预，完全读取数据库
            const { results: myFunds } = await env.DB.prepare("SELECT * FROM funds").all();

            let fundLogs = [];
            try {
                const logsQuery = await env.DB.prepare("SELECT * FROM fund_logs ORDER BY id DESC LIMIT 50").all();
