/**
 * 1nuo 财富中枢 V1.0 (终极流水引擎版)
 * ---------------------------------------
 * 核心升级：
 * 1. 顶部北京时间时钟
 * 2. 基金双表驱动 (funds存储总盘, fund_logs存储流水)
 * 3. 基金调仓完全可撤销化
 */
    ...
    try {
        ...
        if (request.method === 'POST' && url.pathname === '/api/analyze') {
            ...
            const dsRes = await fetch('https://api.deepseek.com/chat/completions', {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${env.DEEPSEEK_API_KEY}`, 'Content-Type': 'application/json' },
                body: JSON.stringify({ model: 'deepseek-reasoner', messages: [{ role: 'system', content: '财务审计官，输出深度Markdown。' }, { role: 'user', content: JSON.stringify(data) }] })
            });
        ...
        if (request.method === 'PUT' && url.pathname.startsWith('/api/fund/')) {
            ...
            const dsRes = await fetch('https://api.deepseek.com/chat/completions', {
                ...
                body: JSON.stringify({
                    model: 'deepseek-chat',
                    messages: [
                        { role: 'system', content: `解析基金调仓。净值:${nav}。加仓为正数，减仓为负数。例如"加仓100元"，则amount=100, shares=100/${nav}。例如"卖出50份"，则shares=-50, amount=-50*${nav}。返回JSON:{"amount":数字, "shares":数字}` }, 
                        { role: 'user', content: text }
                    ],
                    response_format: { type: 'json_object' }
                })
            ...
            const parsed = extractJSON((await dsRes.json()).choices[0].message.content);
            const deltaAmount = parsed.amount || 0;
            const deltaShares = parsed.shares || 0;
        ...
        if (request.method === 'GET') {
            ...
            Object.keys(accounts).forEach(name => {
                ...
                if (acc.type === 'liability') { 
                    ...
                    rowHtml = `<div class="list-row"><span>${name}</span><div style="text-align:right;"><b style="color:#ff4d4f;">-¥${acc.balance.toFixed(2)}</b><br><small style="color:#999;font-size:10px;">可用 ¥${(acc.limit - acc.balance).toFixed(2)} / 总额 ¥${acc.limit}</small> <span class="btn-tiny" onclick="window.editAccount('${name}', ${acc.balance}, ${acc.limit})">🤖 调额</span></div></div>`;
                } else {
                ...
                if (name === '花呗') huabeiHtml = rowHtml; else accountsHtml += rowHtml;
            });
            ...
            <div id="fundModal" style="display:none; position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); background:white; padding:25px; border-radius:24px; box-shadow:0 10px 40px rgba(255, 182, 193, 0.3); z-index:1000; width:80%; max-width:300px;">
                ...
                <textarea id="fundPromptText" placeholder="告诉AI你的操作，例如：今日加仓1000元" style="width:100%; height:80px; margin-bottom:15px;"></textarea>
            ...
            <script>
                ...
                window.undoFundLog = async function(id) {
                    if(confirm('确定要撤销这笔基金操作吗？系统将自动扣减对应的本金和份额！')) { 
                ...
                window.editAccount = async function(name, b, l) {
                    ...
                    if(text) { document.body.style.opacity = '0.5'; await fetch('/api/account/'+encodeURIComponent(name), {method:'PUT', body:JSON.stringify({text})}); location.reload(); }
                };
