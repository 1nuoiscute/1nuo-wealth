  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'POST' && url.pathname === '/api/auth/register') {
        ...
        try {
            ...
            // 初始化默认账户 (如果因重名失败，现在可以通过页面手动添加补救)
            ...
            } catch(e) { console.log('Auto-init skipped, manual add required'); }
    ...
    try {
        ...
        // API：账户管理 (新增功能！)
        // ==========================================
        if (request.method === 'POST' && url.pathname === '/api/manage_account') {
            const { name, type, balance, limit } = await request.json();
            // 使用 INSERT OR IGNORE 防止报错，如果已存在则跳过
            await env.DB.prepare("INSERT OR IGNORE INTO accounts (account_name, type, balance, credit_limit, user_id) VALUES (?, ?, ?, ?, ?)").bind(name, type, Number(balance), Number(limit), userID).run();
            return new Response(JSON.stringify({ success: true }));
        }

        if (request.method === 'DELETE' && url.pathname.startsWith('/api/manage_account/')) {
            const name = decodeURIComponent(url.pathname.split('/').pop());
            await env.DB.prepare("DELETE FROM accounts WHERE account_name = ? AND user_id = ?").bind(name, userID).run();
            return new Response(JSON.stringify({ success: true }));
        }
        ...
        if (request.method === 'GET') {
            ...
            sortedAccounts.forEach(([name, acc]) => {
                ...
                if (acc.type === 'liability') { 
                    ...
                    rowHtml = `<div class="list-row"><span>${name}</span><div style="text-align:right;"><b>-¥${acc.balance.toFixed(2)}</b><br><small style="color:#999;font-size:10px;">可用 ¥${(acc.limit - acc.balance).toFixed(2)} / 总额 ¥${acc.limit}</small> <span class="btn-tiny" style="color:#ccc; border-color:#eee; margin-right:2px;" onclick="window.delAccount('${name}')">[删]</span> <span class="btn-tiny" onclick="window.editAccount('${name}', ${acc.balance}, ${acc.limit})">🤖 调额</span></div></div>`;
                } else {
                    liquidAssets += acc.balance;
                    rowHtml = `<div class="list-row"><span>${name}</span><span><b>¥${acc.balance.toFixed(2)}</b> <span class="btn-tiny" style="color:#ccc; border-color:#eee; margin-right:2px;" onclick="window.delAccount('${name}')">[删]</span> <span class="btn-tiny" onclick="window.editAccount('${name}', ${acc.balance}, ${acc.limit})">🤖 调平</span></span></div>`;
                }
            ...
            if (accounts['花呗']) {
                ...
                huabeiHtml = `<div class="list-row"><span>花呗</span><div style="text-align:right;"><b style="color:#ff4d4f;">-¥${hAcc.balance.toFixed(2)}</b><br><small style="color:#999;font-size:10px;">可用 ¥${(hAcc.limit - hAcc.balance).toFixed(2)} / 总额 ¥${hAcc.limit}</small> <span class="btn-tiny" style="color:#ccc; border-color:#eee; margin-right:2px;" onclick="window.delAccount('花呗')">[删]</span> <span class="btn-tiny" onclick="window.editAccount('花呗', ${hAcc.balance}, ${hAcc.limit})">🤖 调平</span></div></div>`;
            }
            ...
            <html lang="zh-CN">
             <head>
                ...
                <meta name="apple-mobile-web-app-title" content="1nuo 资产管理">
                <link rel="apple-touch-icon" href="https://img.1nuo.me/blog/2026/02/25/79d47ec3bd03f7c7a5bbcb2c952cebb5.webp">
                <title>1nuo 资产管理 V2.1</title>
    ...
    <div onclick="toggleNotice()" style="position:relative; cursor:pointer; font-size:12px; font-weight:normal; background:white; border:1px solid #ffdce0; padding:2px 8px; border-radius:10px; display:flex; align-items:center;">
        📢 公告
        <div id="red-dot" style="display:none; position:absolute; top:-3px; right:-3px; width:8px; height:8px; background:#ff4d4f; border-radius:50%; border:2px solid white;"></div>
    </div>
</div><div id="notice-box" style="display:none; grid-column: 1 / -1; background:rgba(255,255,255,0.6); backdrop-filter:blur(10px); padding:15px; border-radius:20px; border:1px dashed #ffb7c5; margin:5px 0 10px 0; font-size:13px; color:#666; line-height:1.6;">
    <h4 style="margin:0 0 8px 0; color:#ffb7c5; display:flex; justify-content:space-between; align-items:center;">
        <span>💡 系统使用公告</span>
        <span onclick="window.toggleNotice()" style="cursor:pointer; color:#ccc; font-size:16px;">✕</span>
    </h4>
    1. <b>App体验</b>：在浏览器菜单选【添加到主屏幕】，即可像原生 App 一样全屏使用。<br>
    2. <b>利息同步</b>：余额宝利息无法自动抓取，建议每周通过【🤖 调平】手动对齐一次总额。
</div>
                ...
                <div class="right-col">
                    <div class="card" style="background:#fffcf0; border-color:#ffe58f; margin-bottom:15px;">
                        ...
                        ${yuebaoHtml || '<div style="padding:15px; text-align:center; color:#999; background:#fffaf5; border-radius:12px; border:1px dashed #ffe58f;"><small>暂无数据，但你可以点击下方【+ 新增基金】手动添加</small></div>'}
                    </div>
                    <div class="card">
                        <div class="card-header" style="display:flex; justify-content:space-between; align-items:center;">
                            <span>💳 资产分布与详情</span>
                            <span onclick="window.addNewAccount()" style="font-size:12px; cursor:pointer; color:#ffb7c5; border:1px solid #ffb7c5; padding:1px 6px; border-radius:10px;">+ 新增</span>
                        </div>
                        ${accountsHtml}
                    </div>
                </div>
            ...
            <script>
                // 公告版本号（下次你想更新公告让红点再亮，就改一下这个数字）
const NOTICE_VER = '20260225'; 

window.initNotice = () => {
    // 如果本地存储的版本号不对，就显示红点
    if (localStorage.getItem('notice_read') !== NOTICE_VER) {
...
window.toggleNotice = () => {
    ...
    // 如果是打开操作，隐藏红点并记录已读
    if (!isShow) {
        localStorage.setItem('notice_read', NOTICE_VER);
        dot.style.display = 'none';
    }
};

// 页面加载 0.5 秒后检测红点
setTimeout(window.initNotice, 500);
                window.auditData = ${JSON.stringify(finalReport)};
                window.currentEditFund = null; 

                window.addNewAccount = async function() {
                    const name = prompt('请输入账户名称 (例如: 微信):');
                    if(!name) return;
                    const type = prompt('类型 (填 "asset" 代表资产, "liability" 代表负债):', 'asset');
                    const bal = prompt('初始余额:', 0);
                    
                    document.body.style.opacity = '0.5';
                    try {
                        await fetch('/api/manage_account', {
                            method: 'POST', 
                            body: JSON.stringify({ name: name, type: type, balance: bal, limit: 0 }) 
                        });
                        location.reload();
                    } catch(e) { alert('添加失败'); document.body.style.opacity = '1'; }
                };

                window.delAccount = async function(name) {
                    if(confirm('确定要删除账户【'+name+'】吗？')) {
                        document.body.style.opacity = '0.5';
                        await fetch('/api/manage_account/'+encodeURIComponent(name), {method:'DELETE'});
                        location.reload();
                    }
                };
